...
*/
  ...
  //const regexParseTemplate = '${([\w\p{Han}\p{Katakana}\p{Hiragana}]+)(\*)?((?:\s+[\w\p{Han}\p{Katakana}\p{Hiragana}][\p{Han}\p{Katakana}\p{Hiragana}\w-]+=(?:[%]?[\p{Han}\p{Katakana}\p{Hiragana}\w]+|[\'\"\”][^\'\"\”]+[\'\"\”]))*)\s*([^}]*)}$' ;
  ...
  function parseTemplate($layout, $pageAttributes)
  {
    ...
    for($i = 0 ;  $i < count($parsedFields[0]) ; $i++)
    {
      ...
      $attributeMap = EcampaignField::parseAttributes($knownField[1]." ".trim($parsedFields[3][$i]));
      if($parsedFields[4]){
        $parsedFields[4][$i] = str_replace("&#8221;", "\"", $parsedFields[4][$i]);
        
        $oldMap = count($attributeMap);
        $attributeMap = EcampaignField::parseAttributes($knownField[1]." ".trim($parsedFields[4][$i]));

        if(count($attributeMap) > $oldMap){
          $parsedFields[4][$i] = "";
        }
      } 
...
*/
      ...
      case self::sCounter :
        ...
        $val = get_post_meta(4, self::ecCounter, true);
